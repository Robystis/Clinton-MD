...
const botname = process.env.BOTNAME || 'Clinton MD';
const antibot = process.env.ANTIBOT || 'FALSE';
    ...
    const author = process.env.STICKER_AUTHOR ||'Clint™𝕏';
    const packname = process.env.STICKER_PACKNAME || 'clint';
...
const admin = process.env.ADMIN_MSG || '𝐅𝐚𝐢𝐥𝐮𝐫𝐞!, 𝐘𝐨𝐮 𝐚𝐫𝐞 𝐭𝐨𝐨 𝐲𝐨𝐮𝐧𝐠 𝐭𝐨 𝐝𝐨 𝐭𝐡𝐚𝐭🦄';
    ...
    const NotOwner = process.env.NOT_OWNER_MSG || 'Lol you are not 𝑪𝒍𝒊𝒏𝒕𝒐𝒏 𝕏';
...
if (antilink === 'TRUE' && antilinkall === 'TRUE' && body.includes('http') && !Owner && isBotAdmin && !isAdmin && m.isGroup) { 
 ...
 client.sendMessage(m.chat, {text:`Auf wiedersehen:\n\n@${ki.split("@")[0]}, sending links is prohibited in this group!`, contextInfo:{mentionedJid:[ki]}}, {quoted:m}); 
    ...
    if (cmd && !m.isGroup) {
      console.log(chalk.black(chalk.bgWhite("[ 𝑪𝒍𝒊𝒏𝒕𝒐𝒏-AI ]")), color(argsLog, "turquoise"), chalk.magenta("From"), chalk.green(pushname), chalk.yellow(`[ ${m.sender.replace("@s.whatsapp.net", "")} ]`));
...
client.sendMessage(m.chat, { image: { url: 'https://i.imgur.com/T04xUxP.png' }, caption: cap, fileLength: "9999999999"}, { quoted: m })
} else if (menu === 'LINK') {
client.sendMessage(m.chat, {
                        text: cap,
                        contextInfo: {
                            externalAdReply: {
                                ...
                                sourceUrl: 'https://wa.me/254735342808?text=ʜᴇʟʟᴏ+Clinton𝕏+Nihostie+Bot+Mkuu+🥲',
                                mediaType: 1,
...
let xf = m.quoted.text;
                ...
                let pppuser = await client.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/e13333e4152a69d780344.png')
...
try {
 ...
 if (users  == client.decodeJid(client.user.id)) throw 'Bot cannot remove itself itself you moran😡';
...
`👋🏻 Hi *${pushname}*,You can deploy Clinton-MD using the GitHub link below🦄\n\nFork and give us a star✨.\n\n https://github.com/xhclintohn/Clinton-MD\n\nLink with your whatsapp using pairing link below\nhttps://replit.com/@njaisirm1/Crown-Pairing-v6\n\nEnjoy and have fun with Clinton 👑..\n\nMade on Earth by 𝑪𝒍𝒊𝒏𝒕𝒐𝒏𝕏!!` });
 ...
 break;
          case "leave": { 
                 if (!isAdmin) throw admin; 
 await client.sendText(m.chat, 'GoodBye Everyone. Bot is leaving now goodbye all😙⚡. . .'); 
        ...
        try {
            ...
            if (fileSizeInMegabytes <= 100) {
                client.sendMessage(
                    from, {
                        video: fs.readFileSync(`./${randomName}`),
                        caption: `𝐆𝐞𝐧𝐞𝐫𝐚𝐭𝐞𝐝 𝐛𝐲 𝐂linton-Bot`,
                    }, {
                        quoted: m
                    }
                );
...
case "movie": 
 ...
 client.sendMessage(m.chat, { video: { url: 'https://i.imgur.com/9WyMGc4.mp4' }, caption: `Say no more 😌 here is Clintons contact +254735342808 🥱` }, {quoted: m}); 
 ...
 break;
    ...
    break;
         ...
         for (let i of res) { 
             let txt = `*_</ 𝑪𝒍𝒊𝒏𝒕𝒐𝒏  𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 >\n\n🀄 Message: ${text}\n\nAuthor: ${pushname}_*` 
             await client.sendMessage(i, { 
                 image: { 
                     url: "https://i.imgur.com/T04xUxP.png" 
                 }, 
                 caption: `${txt}` 
             }) 
         } 
...
case "getvar": 
 ...
 break;
        ...
        case "gpt": case "g": 
            ...
            if (!text) return reply("Hello am 𝑪𝒍𝒊𝒏𝒕𝒐𝒏 md an Ai developed by 𝑪𝒍𝒊𝒏𝒕𝒐𝒏𝕏, how can I help you today?");
